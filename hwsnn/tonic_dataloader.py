...
class DatasetLoader():
    ...
    def __init__(self, dataset='shd', caching='disk', num_workers=0, batch_size = 256, time_window=50):
        ...
        elif dataset == 'nmnist_784':

            sensor_size = tonic.datasets.NMNIST.sensor_size
            frame_transform = transforms.ToFrame(sensor_size=(17,17,2), n_time_bins=time_window)
            #polarity_transform = tonic.transforms.MergePolarities()
            #downsample_transform = transforms.Downsample(spatial_factor = 0.8235)
            downsample_transform = transforms.Downsample(spatial_factor = 0.5)
