...
class CopyMemoryDataLoader(data.Dataset):
    """
    Copy Memory Dataloader class
    """

    def __init__(self, n_samples, t_length, mem_length=10):
        """
        ...
        """
        super(CopyMemoryDataLoader, self).__init__()
    ...
    def __getitem__(self, idx):
        ...
        if idx < self.n_samples:
            _x, _y = self.create_sample(self.t_length, self.mem_length, idx)
            return _x, _y

        else:
            sys.exit('\nERROR: Sample index exceeds the number of samples in '
                     'dataset')

    @staticmethod
    def create_sample(t_length, mem_length, idx):
        """
        ...
        seq_length = t_length + 2 * mem_length + 1
        x = torch.zeros([1, seq_length], dtype=torch.int32)
        y = torch.zeros([1, seq_length], dtype=torch.int32)

        torch.manual_seed(idx)
        x[0, 0:mem_length] = torch.randint(1, 9, (1, mem_length))
        x[0, -mem_length-1:] = 9

        y[0, -mem_length:] = x[0, 0:mem_length]
