...
from hwsnn.abstract_rsnn import Abstract_SNN_Delays
import json
import numpy as np
...
class RSNN_d_d(Abstract_SNN_Delays, nn.Module):
    ...
    def mem_update_out(self, i_spike, o_spike, mem):
        ...
        mem = mem*(mem < self.th_reset) # comment out for adding task
        return mem, o_spike
    ...
    def mem_update_rnn(self, i_spike, o_spike, mem):
        ...
        mem = a + b + c
        o_spike = self.act_fun(mem-self.thresh)
        mem = mem*(mem < self.thresh) # comment out for adding task
        return mem, o_spike
