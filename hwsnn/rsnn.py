...
from hwsnn.abstract_rsnn import Abstract_SNN
...
class RSNN_2l(Abstract_SNN, nn.Module):
    ...
    def define_operations(self):
        ...
        if self.tau_m != 'adp':
            ...
            #a = torch.logit(torch.exp(-1/torch.tensor(self.win/50))).item()
            #b = torch.logit(torch.exp(-1/torch.tensor(self.win))).item()
            
            #a = self.win/50
            #b = self.win
            rate = 10/self.win # rate 1->10
            concentration = 2.0           
            self.tau_m_h_1 = nn.Parameter(torch.distributions.gamma.Gamma(torch.ones(self.num_hidden) * concentration ,torch.ones(self.num_hidden) * rate ).sample())
            self.tau_m_h_2 = nn.Parameter(torch.distributions.gamma.Gamma(torch.ones(self.num_hidden) * concentration ,torch.ones(self.num_hidden) * rate ).sample())
            self.tau_m_o = nn.Parameter(torch.distributions.gamma.Gamma(torch.ones(self.num_output) * concentration ,torch.ones(self.num_output) * rate ).sample())
            ...
            #nn.init.uniform_(self.tau_m_h_1, a, b)
            #nn.init.uniform_(self.tau_m_h_2, a, b)
            #nn.init.uniform_(self.tau_m_o, a, b)
    ...
    def forward(self, input):
        ...
        for step in range(self.win):
            ...
            h1_mem, h1_spike = self.mem_update_rnn(
                i_spike, h1_spike, h1_mem, self.fc_ih, self.fc_h1h1, self.tau_m_h_1)
            h2_mem, h2_spike = self.mem_update_rnn(
                h1_spike, h2_spike, h2_mem, self.fc_h1h2, self.fc_h2h2, self.tau_m_h_2)
    ...
    def mem_update_rnn(self, i_spike, o_spike, mem, op_input, op_recurrent, tau_m):
        #alpha = torch.sigmoid(tau_m).to(self.device)
        alpha = torch.exp(-1/tau_m).to(self.device)
...
class RSNN_2l_ALIF(RSNN_2l):
    ...
    def define_operations(self):
        ...
        if self.tau_m != 'adp':
            ...
            self.tau_m_h_2 = nn.Parameter(torch.Tensor(self.num_hidden))
            self.tau_m_o = nn.Parameter(torch.Tensor(self.num_output))
            nn.init.normal_(self.tau_m_h_1, 20, 5)
            nn.init.normal_(self.tau_m_h_2, 20, 5)
        ...
        self.tau_adp_2 = nn.Parameter(torch.Tensor(self.num_hidden))
        self.tau_adp_o = nn.Parameter(torch.Tensor(self.num_output))
        nn.init.normal_(self.tau_adp_1, 150, 10)  # check this
        nn.init.normal_(self.tau_adp_2, 150, 10)
    ...
    def mem_update_rnn(self, i_spike, o_spike, mem, d, op_input, op_recurrent, tau_m):
        ...
        # bojian way
        alpha = torch.exp(-1/tau_m[0]).to(self.device)
        ro = torch.exp(-1/tau_m[1]).to(self.device)      
