...
TONIC_DATASET_PATH = os.path.join(
    os.environ.get('SNN_DATASETS_PATH'), 'Datasets')

CACHE_PATH = os.path.join(os.environ.get('SNN_DATASETS_PATH'), 
                          'DiskCachedDatasets')
...
class DatasetLoader():
    ...
    def __init__(self, dataset='shd', caching='disk', num_workers=0, batch_size = 256, time_window=50):
        ...
        if 'shd' in dataset:
            ...
            original_sensor_size = tonic.datasets.SHD.sensor_size
            sensor_size_list = list(original_sensor_size)
            spatial_factor = None
            frame_transforms = list()
            
            if dataset == "shd140":
                sensor_size_list[0] = 140
                target_size = (140, 1)
                spatial_factor = np.asarray(target_size) / original_sensor_size[:-1]

            if spatial_factor is not None:
                frame_transforms.append(
                    transforms.Downsample(spatial_factor=spatial_factor[0]))

            frame_transforms.append(transforms.ToFrame(sensor_size=tuple(sensor_size_list), n_time_bins=time_window))
            ...
            test_dataset = tonic.datasets.SHD(save_to=TONIC_DATASET_PATH,
                                            train=False,
                                            transform= transforms.Compose(frame_transforms),
        ...
        if caching=='disk':
            train_cache_path = os.path.join(CACHE_PATH, 'fast_dataloading_{}_train{}'.format(dataset, time_window))
            test_cache_path = os.path.join(CACHE_PATH, 'fast_dataloading_{}_test{}'.format(dataset, time_window))
